...
import useOtpMutation from 'src/http/auth/useOtpMutation';
import useResendMutation from 'src/http/auth/useResendMutation';
...
export function OtpVerification() {
    ...
    const { mutateAsync: resendMutate, isPending: resendPending } = useResendMutation();
        ...
        <>
            ...
            <Stack spacing={1} sx={{ mt: 3, mb: 5, textAlign: 'center', whiteSpace: 'pre-line' }}>
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    {`We've emailed a 6-digit confirmation code. \nPlease enter the code in the box below to verify your account.`}
                </Typography>
            </Stack>
        </>
    );
    ...
    const renderForm = (
        <Stack spacing={3}>
            ...
            <Typography variant="body2" sx={{ mx: 'auto' }}>
                ...
                <Link
                    variant="subtitle2"
                    onClick={() => handleResendCode(adminId)}
                    sx={{
                        cursor: 'pointer',
      ...
      >
        ...
        width={16} icon="eva:arrow-ios-back-fill" />
        Return to sign in
      </Link> */}
        </Stack>
    );
